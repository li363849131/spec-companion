...
export const DocumentOutline: React.FC<DocumentOutlineProps> = ({
  ...
  const renderItem = (item: OutlineItem) => {
    ...
    const isCached = cachedPages.has(item.pageNumber) || cachedChapters.has(item.id);
    const hasChildren = item.children && item.children.length > 0;
    const isCollapsed = collapsedItems.has(item.id);

    // Debug logging
    if (isNaN(item.pageNumber) || item.pageNumber < 1) {
      console.warn('Invalid page number in outline:', item.title, 'pageNumber:', item.pageNumber, item);
    }
    ...
    return (
      <div key={item.id} className="w-full">
        <div
          onClick={() => {
            console.log('Outline item clicked:', item.title, 'pageNumber:', item.pageNumber);
