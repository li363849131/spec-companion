...
export const PdfViewer: React.FC<PdfViewerProps> = ({
  ...
  useEffect(() => {
    ...
    if (doc.category === 'custom' && (doc.pdfBuffer || doc.pdfData)) {
  ...
  const renderPdfPage = useCallback(async () => {
    ...
    // Cancel any ongoing render task on canvas to prevent collision
    ...
    try {
      const validPage = Math.max(1, Math.min(currentPage, pdfDoc.numPages || 1));
  ...
  const handleContinuousScroll = useCallback(() => {
    ...
    const triggerOffset = containerRect.top + 140; // Detection threshold line
  ...
  return (
    ...
    >
      ...
      >
        {doc.category === 'custom' ? (
          // Custom Uploaded PDF via PDF.js Canvas
          <div 
          ...
          >
            {/* Loading / Rendering Indicator Overlay */}
          ...
          // Preset Spec Rich Viewport: Continuous scroll stream OR Single page view
          (internalViewMode === 'continuous' && presetSpec?.pages && presetSpec.pages.length > 0) ? (
            // Continuous Stream of all preset pages
              ...
              {presetSpec.pages.map((pg) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="border-b border-slate-200 dark:border-slate-800 pb-4">
                      <div className="flex items-center justify-between text-xs text-slate-600 dark:text-slate-400 font-mono mb-1.5">
                        ...
                        <span className={`px-2 py-0.5 rounded font-bold ${
                          isCurrent ? 'bg-indigo-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300'
                        }`}>
                    ...
                    {/* Hardware Architecture Diagram / Timing Wave / Bitfield */}
                    {pg.diagramSvg && (
                      <div className="p-4 rounded-lg bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800 shadow-inner">
                        <div className="flex items-center justify-between text-[11px] font-semibold text-slate-600 dark:text-slate-400 mb-2">
                          <span>FIGURE {pg.sectionNumber}-1: HARDWARE LOGIC / BITFIELD SCHEMATIC</span>
                          <span className="text-[10px] bg-slate-200 dark:bg-slate-800 px-1.5 py-0.5 rounded text-slate-700 dark:text-slate-300 font-mono">
                            Vector Interactive
                          </span>
                        </div>
                        <div
                          className="w-full overflow-x-auto"
                          dangerouslySetInnerHTML={{ __html: pg.diagramSvg }}
                        />
                      </div>
                    )}
                    ...
                    {/* Spec Table if available */}
                    {pg.tableData && (
                      <div className="overflow-hidden rounded-md border border-slate-200 dark:border-slate-800">
                        <div className="bg-slate-100 dark:bg-slate-800/80 px-3 py-2 text-xs font-bold text-slate-700 dark:text-slate-300">
                          {pg.tableData.title}
                        </div>
            ...
            >
              ...
              {presetPageData?.diagramSvg && (
                <div className="p-4 rounded-lg bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800 shadow-inner">
                  ...
                  <div
                    className="w-full overflow-x-auto"
                    dangerouslySetInnerHTML={{ __html: presetPageData.diagramSvg }}
                  />
                </div>
              )}
              ...
              {presetPageData?.tableData && (
                <div className="overflow-hidden rounded-md border border-slate-200 dark:border-slate-800">
                  <div className="bg-slate-100 dark:bg-slate-800/80 px-3 py-2 text-xs font-bold text-slate-700 dark:text-slate-300">
                    {presetPageData.tableData.title}
                  </div>
                  <table className="w-full text-xs text-left">
                    <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400 border-b border-slate-200 dark:border-slate-800">
                      <tr>
                        {presetPageData.tableData.headers.map((h, i) => (
                          <th key={i} className="p-2.5 font-semibold">
                            {h}
                          </th>
                        ))}
